...
> 用法：在 Stage 3 自审通过、用户审之前（或之后），可派 `general-purpose` 子代理做一次独立深审。
> 调用时机：Stage 3 文档复杂、模块数 ≥ 5，或用户主动要求二次复核。

```
Agent tool (subagent_type: "general-purpose"):
  description: "DLD 文档深审"
  prompt: |
    ...
    **上游参考**：
    ...
    | Contracts | 每个 public 接口五要素（签名/前置/后置/错误码/幂等）齐全 |
    | State Machine Coverage | 状态机标注 entry/exit/非法事件处理；非法转移有显式拒绝逻辑 |
    | Error Code Closure | 全局错误码表与接口错误码相互一致；每个错误码至少 1 条时序图触发路径 |
    | Derivation Trace | Stage 1 业务规则、聚合根、领域事件、Stage 2 模块清单、外部依赖、跨切面策略全部能在 DLD 中找到对应位置 |
...
**子代理返回后**：
- Status = Approved → 继续 Stage 3 用户审 prompt
